...
import org.rebix.stars.stats.SStatTypeDisplay
import org.rebix.stars.utility.LoreBuilder
import java.util.*
                ...
                if (gemstoneSlotsString.isNotEmpty()) {
                    try {
                        ...
                        slotsList.forEach { slot ->
                            ...
                            if (parts.size == 3) {
                                val type: SGemstoneSlotType = SGemstoneSlotType.valueOf(
                                    parts[0].split("=")[1].trim().uppercase(Locale.getDefault())
                                )
                                var gemstoneType: SGemstoneType? = null
                                try {
                                    gemstoneType = SGemstoneType.valueOf(
                                        parts[1].split("=")[1].trim().uppercase(Locale.getDefault())
                                    )
                                } catch (_: Exception) {
